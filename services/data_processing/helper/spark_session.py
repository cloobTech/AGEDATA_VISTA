...
class SparkDataLoader:
    ...
    def load_from_url(self, url: str, file_type: str = "auto", **kwargs) -> Any:
        ...
        try:
            ...
            if file_type == "auto":
                ...
                file_type = file_ext if file_ext in ['csv', 'json', 'parquet'] else 'csv'
            ...
            df = self._load_from_file(tmp_path, file_type, **kwargs)
            
            # Clean up temp file
            os.unlink(tmp_path)
            
            print(f"✅ Successfully loaded {df.count()} rows from URL")
            return df
    ...
    def load_from_database(self, 
        ...
        try:
            ...
            print(f"✅ Successfully loaded {df.count()} rows from database table: {table}")
            return df
    ...
    def load_from_cloud_storage(self, 
        ...
        try:
            ...
            if file_type == 'parquet':
                ...
                df = self.spark.read.csv(cloud_path, header=True, inferSchema=True)
            elif file_type == 'json':
                df = self.spark.read.json(cloud_path)
            else:
                raise ValueError(f"Unsupported file type for cloud storage: {file_type}")
            
            print(f"✅ Successfully loaded {df.count()} rows from cloud storage: {cloud_path}")
            return df
    ...
    def load_from_hdfs(self, hdfs_path: str, file_type: str = "parquet") -> Any:
        ...
        """
        try:
            full_path = f"hdfs://{hdfs_path}" if not hdfs_path.startswith('hdfs://') else hdfs_path
            ...
            if file_type == 'parquet':
                ...
                raise ValueError(f"Unsupported file type for HDFS: {file_type}")
            
            print(f"✅ Successfully loaded {df.count()} rows from HDFS: {full_path}")
            return df
